...
async def submit_answer(
    ...
    # Get rubrics for the question
    rubrics_result = await db.execute(
        select(Rubric).where(Rubric.question_id == answer_data.question_id)
    )
    ...
    if not rubrics:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="No rubrics found for this question"
        )
    
    # Build rubric string from all rubrics
    rubric_text = "\n".join([f"- {r.criterion} (weight: {r.weight})" for r in rubrics])
